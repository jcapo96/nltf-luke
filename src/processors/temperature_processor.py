...
class TemperatureProcessor(BaseDataProcessor):
    ...
    This processor handles temperature signals and provides methods to:
    - Calculate initial and final temperature values
    - Analyze temperature changes during experiments
    - Plot temperature data with integration windows
    """
    ...
    def plot_temperature(self, start_time: pd.Timestamp, end_time: pd.Timestamp,
        ...
        time_seconds = (temp_data.index - start_time).total_seconds()

        ax.plot(time_seconds, temp_data, "-o",
